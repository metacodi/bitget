...
const testMarketWs = async () => {
  try {
    ...
    ws.accountUpdate().subscribe((data: any) => writeLog(`${data.arg.channel}_${data.arg.instType}_${unixTime()}`, data, `log/trade-03-${market}.ts`));
    ws.orderUpdate().subscribe((data: any) => writeLog(`${data.arg.channel}_${data.arg.instType}_${unixTime()}`, data, `log/trade-03-${market}.ts`));
